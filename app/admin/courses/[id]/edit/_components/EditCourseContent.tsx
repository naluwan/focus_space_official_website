...
const EditCourseContent = ({ session, course }: EditCourseContentProps) => {
  ...
  useEffect(() => {
    if (course) {
      setFormData({
        ...
        price: course.price,
        maxParticipants: course.maxParticipants || 1,
        instructor: course.instructor || '',
        imageUrl: course.imageUrl || '',
        features: course.features.length > 0 ? course.features : [''],
        requirements: course.requirements || '',
        startDate: formatDateToLocalString(new Date(course.startDate)),
        endDate: formatDateToLocalString(new Date(course.endDate)),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const oldImageUrl = formData.imageUrl; // Store old image URL for deletion
        ...
        if (uploadResult && uploadResult[0]) {
          ...
          if (oldImageUrl && oldImageUrl !== imageUrl) {
            try {
              ...
              // Don't fail the whole update if old image deletion fails
            }
          }
        } else {
      ...
      if (response.ok) {
        ...
        if (result.conflicts && result.conflicts.length > 0) {
          ...
          setErrors({ submit: result.message || '更新課程失敗，請稍後再試' });
        }
      }
    } catch (error) {
      console.error('Error updating course:', error);
      setErrors({ submit: error instanceof Error ? error.message : '更新課程失敗，請稍後再試' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className='flex-1 flex flex-col overflow-hidden lg:ml-0'>
        ...
        <header className='bg-white border-b border-gray-200 px-6 py-4'>
          <div className='flex items-center justify-between'>
            ...
            <h1 className='text-2xl font-bold text-gray-900'>編輯課程</h1>
          </div>
        </header>
                  ...
                  >
                    {loading || isUploading ? (
                      <>
                        ...
                        <span>{isUploading ? '上傳圖片中...' : '更新中...'}</span>
                      </>
                    ) : (
                      <>
                        <Save size={16} />
